...
/**
 * Interpolates a {@link PositionObject}'s right rotation from {@code start} to {@code end} over
 * {@code duration} ticks, passed through an ease-in-out-sine curve before the spherical
 * interpolation.
 * <p>
 * See {@link RRotationAnimation} for the linear equivalent, and {@link RotationAnimationSmooth}
...
public class RRotationAnimationSmooth extends AAnimationInterpolationFunction<Quaternionf> {
    ...
    public void nextTick(int duration, int tick, Quaternionf startRotation, Quaternionf endRotation, PositionObject obj) {
        ...
        if (tick == 0) {
            ...
            // transformation interpolation can glitch on restart instead of starting cleanly.
